...
export class Snake {
    ...
    constructor(map: Array<any>[], color: number, copy:boolean = false) {
        if(copy) return;
    ...
    public increase(turn: number) {
        if (turn <= 10 || (turn - 10) % 3 === 0) {
            let len = this.body.length - 1;
            let node: [number, number] = [this.body[len][0], this.body[len][1]];
            this.body.push(node);
        }
    }
    ...
    public checkKeyBoard(d: string): boolean {
        let dir: number = -1;
        switch (d) {
            case "w":
                dir = UP;
