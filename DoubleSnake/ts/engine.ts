...
export class Engine {
    ...
    private view: View = new View(this.map);
    private snake_black: Snake;
    private snake_white: Snake;
    private turn: number = 0;
    private cur_player = false;
    private game_mode = "alpha_beta";
    ...
    private getResult(): any {
        if(this.game_mode==="alpha-beta") return ab.abSearch(this.map, this.snake_black, this.snake_white, this.turn, this.cur_player);
        else if(this.game_mode==="min-max") return MinMax.minMaxSearch(this.map, this.snake_black, this.snake_white, this.turn, this.cur_player);
        else return MCTS.monte_carlo_tree_search(this.cur_player, this.snake_white, this.snake_black, this.turn);
    }

    public resetGameMode(mode: string) {
        this.game_mode = mode;
    }
