...
rl.on('line', function (line: any) {
    ...
    let obsCount = input.requests[0]["obstacle"]?.length;
    // 处理地图障碍物
...
function getDire(dir: number): number {
    switch (dir) {
        case 0:
            return LEFT;
        case 1:
...
function reverseDire(dir: number): number {
    switch (dir) {
        case 0:
            return 3;
        case 1:
...
class Snake {
    public body: Array<[number, number]> = [];
    public map: Array<any>[] = [];
    public head_color: number = config.BLACK;
    public body_color: number = config.WHITE;
    public direction: number = UP;

    constructor(map: Array<any>[], color: number, copy:boolean) {
        if(copy) return;
    ...
    // go!
    public go(turn: number) {
        // move head
        let head: [number, number] = [this.body[0][0], this.body[0][1]];
    ...
    // delete one move
    public goBack(turn: number, tail: [number, number]) {
        // remove head
    ...
    private checkMove(direction: number): boolean {
        let x = this.body[0][0];
        let y = this.body[0][1];
        switch (direction) {
            case UP:
    ...
    public evaluate(): number {
        let val = 0;
        let queue: Array<[number, number]> = [], head = 0, tail = 0;
        let vis: Array<number>[] = [];
        for (let i = 0; i < height; i++) {
            let row = [];
            for (let j = 0; j < width; j++) {
                row.push(0);
        ...
        let u: [number, number] = [this.body[0][0], this.body[0][1]];
        queue[tail++] = u;
        while (head !== tail) {
            let top = queue[head];
            head++;
    ...
    public genLegalMoves(): Array<number> {
